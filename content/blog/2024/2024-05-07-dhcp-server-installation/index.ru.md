...
lastmod: 2026-09-13T20:52:00+03:00
...
## <span class="section-num">4</span> Хуки {#хуки}

| Имя библиотеки           | Назначение                                                 |
|--------------------------|------------------------------------------------------------|
| `libddns_gss_tsig.so`    | Подписывает DNS-обновления с помощью GSS-TSIG.             |
| `libdhcp_bootp.so`       | Обрабатывает запросы BOOTP в соответствии с RFC1497.       |
| `libdhcp_class_cmds.so`  | Управляет клиентскими классами через API.                  |
| `libdhcp_ddns_tuning.so` | Настраивает динамические DNS-обновления (DDNS).            |
| `libdhcp_flex_id.so`     | Создает гибкие идентификаторы для резервирования хостов.   |
| `libdhcp_flex_option.so` | Динамически задает значения опций DHCP.                    |
| `libdhcp_ha.so`          | Обеспечивает высокую доступность (отказоустойчивость).     |
| `libdhcp_host_cache.so`  | Кэширует данные о хостах для повышения производительности. |
| `libdhcp_host_cmds.so`   | Управляет резервированиями хостов через API.               |
| `libdhcp_lease_cmds.so`  | Управляет арендованными адресами (lease) через API.        |
| `libdhcp_lease_query.so` | Поддерживает запросы Leasequery.                           |
| `libdhcp_legal_log.so`   | Ведет журнал аудита для юридических целей.                 |
| `libdhcp_limits.so`      | Ограничивает выделение адресов и обработку пакетов.        |
| `libdhcp_mysql.so`       | Бэкенд для хранения резервирований хостов в MySQL.         |
| `libdhcp_perfmon.so`     | Мониторит производительность обработки пакетов.            |
| `libdhcp_pgsql.so`       | Бэкенд для хранения резервирований хостов в PostgreSQL.    |
| `libdhcp_ping_check.so`  | Проверяет доступность IPv4-адреса перед его выдачей.       |
| `libdhcp_radius.so`      | Взаимодействует с серверами RADIUS.                        |
| `libdhcp_run_script.so`  | Запускает внешние скрипты при событиях DHCP.               |
| `libdhcp_stat_cmds.so`   | Предоставляет команды для получения статистики аренды.     |
| `libdhcp_subnet_cmds.so` | Управляет подсетями и общими сетями через API.             |

-   `libddns_gss_tsig.so`
    -   Позволяет серверу `kea-dhcp-ddns` использовать GSS-TSIG для подписи DNS-обновлений.
    -   Необходимо для безопасного динамического обновления DNS-записей.

-   `libdhcp_bootp.so`
    -   Обеспечивает обработку запросов BOOTP в соответствии с RFC1497.
    -   Старые запросы BOOTP без расширений (RFC951) не поддерживаются.
    -   Классифицирует такие запросы и соответствующим образом формирует ответ.

-   `libdhcp_class_cmds.so`
    -   Набор команд для управления клиентскими классами в конфигурации сервера DHCP без его перезапуска.
    -   Позволяет добавлять, обновлять, удалять и просматривать классы.

-   `libdhcp_ddns_tuning.so`
    -   Настройка динамических DNS-обновлений (DDNS).
    -   Позволяет пропускать DDNS-обновления для определенных клиентов, если они входят в специальный класс `SKIP_DDNS`.

-   `libdhcp_flex_id.so`
    -   Гибкие идентификаторы для резервирования хостов.
    -   Вместо стандартных (MAC-адрес, DUID) можно использовать выражение, которое вычисляет идентификатор на основе различных параметров запроса клиента.

-   `libdhcp_flex_option.so`
    -   Возможность динамически задавать значения опций DHCP в ответах сервера.
    -   Вместо статических значений можно использовать выражения, которые вычисляются на основе параметров входящего запроса.

-   `libdhcp_ha.so`
    -   Организация высокой доступности (High Availability).
    -   Загружается на пару серверов DHCPv4 или DHCPv6 и обеспечивает их совместную работу, чтобы при отказе одного сервера другой мог продолжить обслуживание клиентов.
    -   Поддерживает режимы load-balancing, hot-standby и passive-backup.

-   `libdhcp_host_cache.so`
    -   Кэширует информацию о резервированиях хостов, полученную из медленных баз данных (например, RADIUS), для повышения производительности.
    -   Используется для работы механизма учета RADIUS.

-   `libdhcp_host_cmds.so`
    -   Команды для управления резервированиями хостов через API.
    -   Позволяет добавлять, удалять, обновлять и запрашивать информацию о резервированиях без перезапуска сервера.

-   `libdhcp_lease_cmds.so`
    -   Библиотека команд для управления арендованными адресами (lease).
    -   Необходима для работы внешних систем управления и для функционирования хука высокой доступности `libdhcp_ha.so`.

-   `libdhcp_lease_query.so`
    -   Добавляет поддержку протоколов:
        -   DHCPv4 Leasequery (RFC 4388);
        -   DHCPv4 Bulk Leasequery (RFC6926);
        -   DHCPv6 Leasequery (RFC 5007);
        -   DHCPv6 Bulk Leasequery (RFC5460).

-   `libdhcp_legal_log.so`
    -   Ведение подробного журнала аудита всех назначений и продлений аренды (для юридических целей).

-   `libdhcp_limits.so`
    -   Позволяет ограничивать выделение аренды и обработку пакетов.
    -   Например, можно запретить одному клиенту получать несколько IP-адресов из одной подсети.

-   `libdhcp_mysql.so`
    -   Бэкенд для хранения резервирований хостов в базе данных MySQL.
    -   Начиная с Kea 2.7.4, эту библиотеку необходимо явно загружать для использования MySQL в качестве хранилища резервирований.

-   `libdhcp_perfmon.so`
    -   Мониторинг производительности.
    -   Собирает и логирует данные о времени обработки пакетов на различных этапах.

-   `libdhcp_pgsql.so`
    -   Бэкенд для хранения резервирований хостов в базе данных PostgreSQL.
    -   Начиная с Kea 2.7.4, эту библиотеку необходимо явно загружать для использования PostgreSQL в качестве хранилища резервирований.

-   `libdhcp_ping_check.so`
    -   Добавляет возможность выполнять ping-проверку кандидатного IPv4-адреса перед его предложением клиенту.
    -   Помогает избежать конфликтов IP-адресов.
    -   Проверка выполняется с помощью ICMP ECHO.

-   `libdhcp_radius.so`
    -   Предоставляет поддержку клиента RADIUS.
    -   Позволяет Kea взаимодействовать с серверами RADIUS для авторизации и учета.
    -   Для корректной работы требует загрузки `libdhcp_host_cache.so`.

-   `libdhcp_run_script.so`
    -   Позволяет запускать внешние скрипты при наступлении определенных событий DHCP (например, выдача, продление или истечение аренды).

-   `libdhcp_stat_cmds.so`
    -   Дополнительные команды для получения статистики по аренде.

-   `libdhcp_subnet_cmds.so`
    -   Библиотека команд для управления подсетями и общими сетями.
    -   Позволяет динамически (без перезапуска) добавлять, удалять, изменять и просматривать конфигурацию подсетей и общих сетей.


## <span class="section-num">5</span> Подготовка {#подготовка}


### <span class="section-num">5.1</span> Брандмауэр {#брандмауэр}
...
## <span class="section-num">6</span> Подготовка базы данных {#подготовка-базы-данных}


### <span class="section-num">6.1</span> PostgreSQL {#postgresql}

-   Проверьте, есть ли поддержка PostgreSQL:
    ```shell
    kea-dhcp4 -V
    ```

    -   В строке бэкендов вы увидите слово PostgreSQL.
    -   В kea-3 поддержка базы данных находится в `hooks`.
...
#### <span class="section-num">6.1.1</span> Создание базы данных {#создание-базы-данных}
...
#### <span class="section-num">6.1.2</span> Заполнение базы данных {#заполнение-базы-данных}
...
#### <span class="section-num">6.1.3</span> Обновление базы данных {#обновление-базы-данных}
...
### <span class="section-num">6.2</span> Миграция с ISC DHCP на ISC Kea {#миграция-с-isc-dhcp-на-isc-kea}
...
### <span class="section-num">6.3</span> Тьюнинг базы данных {#тьюнинг-базы-данных}
...
### <span class="section-num">6.4</span> Конфигурация базы данных для Kea {#конфигурация-базы-данных-для-kea}
...
-   Добавим конфигурацию базы данных:
    ...
    ```
-   В kea-3 нужно подключить хук для PostgreSQL:
    ```json
    {
      "Dhcp4": {
        "hooks-libraries": [
...
## <span class="section-num">7</span> Виртуальные интерфейсы {#виртуальные-интерфейсы}

-   Для подключения к сети использовал интерфейсы VLAN (см. [Linux. Настройка vlan]({{< relref "2024-05-13-linux-vlan-configuration" >}}))


### <span class="section-num">7.1</span> Миграция интерфейсов {#миграция-интерфейсов}
...
### <span class="section-num">7.2</span> Конфигурация интерфейсов для Kea {#конфигурация-интерфейсов-для-kea}
...
## <span class="section-num">8</span> Управляющий агент kea-ctrl-agent {#управляющий-агент-kea-ctrl-agent}


### <span class="section-num">8.1</span> По умолчанию {#по-умолчанию}
...
### <span class="section-num">8.2</span> Доступ по сети {#доступ-по-сети}
...
#### <span class="section-num">8.2.1</span> Брандмауэр {#брандмауэр}
...
#### <span class="section-num">8.2.2</span> Сертификаты {#сертификаты}

-   В качестве сертификатов можно использовать сертификаты ACME.
-   Если сертификаты не установлены, то используется протокол `http`.


#### <span class="section-num">8.2.3</span> Конфигурационный файл {#конфигурационный-файл}
...
#### <span class="section-num">8.2.4</span> Пример запроса {#пример-запроса}
...
## <span class="section-num">9</span> Запуск {#запуск}
...
## <span class="section-num">10</span> Утилиты {#утилиты}


### <span class="section-num">10.1</span> Просмотр арендованных адресов {#просмотр-арендованных-адресов}

-   Арендованные адреса можно смотреть в базе данных.
-   Для разных баз данных существуют конкретные утилиты.


#### <span class="section-num">10.1.1</span> Postgres {#postgres}
...
## <span class="section-num">11</span> После установки {#после-установки}
