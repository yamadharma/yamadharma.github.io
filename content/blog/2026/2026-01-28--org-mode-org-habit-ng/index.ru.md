...
lastmod: 2026-07-23T20:38:00+03:00
tags: ["org-mode", "emacs"]
...
## <span class="section-num">5</span> Редактирование {#редактирование}

-   Для заголовка, описывающего привычку, необходимы две вещи:
    -   свойство `:STYLE: habit`;
    -   метка времени `SCHEDULED`. Повторитель в метке времени не требуется.


### <span class="section-num">5.1</span> Панель редактирования {#панель-редактирования}

-   Создаёт новую привычку.
-   Чтобы изменить привычку, которая уже имеет правило `RECURRENCE`, используйте мастер, который сначала считывает существующее правило.
-   Наведите курсор на заголовок, описывающий привычку, откройте панель:
    ```emacs-lisp
    M-x org-habit-ng-author
    ```

-   Получим меню.
-   Также предварительный просмотр в реальном времени правила, которое вы создаете, плюс несколько следующих реальных дат.
    -   Данные обновляется каждый раз, когда вы нажимаете клавишу, поэтому вы видите эффект каждого выбора по мере его совершения.

-   Выбор модели повтора (`s`):
    -   по расписанию (On a schedule);
    -   несколько раз за период (So many times each period);
    -   Хотя бы через определенные промежутки времени (At least every so often);
    -   Несколько раз в течение скользящего окна (So many times in a rolling window).
-   Для привычки, выполняемой по расписанию:
    -   `f` : изменяет частоту;
    -   `i` : устанавливает интервал;
    -   `d` : открывает многофункциональный переключатель для будних дней (еженедельный);
    -   `m` : открывает подменю дня/порядкового номера месяца;
    -   `o` : подменю года;
    -   `l` / `y` : установить поздний/ранний допуск (`X-FLEX-AFTER` / `X-FLEX-BEFORE`);
    -   `~` : переключает ближайший день недели (`X-SKIP`), отображается только в том случае, если правило является ежемесячным/ежегодным правилом выбора дня месяца;
    -   `e` : устанавливает границу `COUNT` или  `UNTIL`;
    -   `n` : временной интервал для сезонных месяцев;
    -   `k` : добавляет диапазон отпусков;
    -   `g` : загружает рабочий пример;
    -   `RET` : сохраняет расписание;
    -   `q` : сбрасывает панель.

-   Конфигурация месяцев.
    -   В подменю месяца (`m`) можно получить доступ к набору порядковых дней недели, а не одному единственному (с помощью пошагового интерфейса).
    -   Переместите курсор порядкового номера с помощью `1` `2` `3` `4` `l` (последний), затем нажмите букву дня недели (`u m t w h f s` = Вс..Сб), чтобы переключать эту пару порядковый номер-день недели.

-   Например: установите курсор на `2` и нажмите `t`, затем установите его на `4` и нажмите `t` еще раз, и панель будет сформирована:
    ```org
    FREQ=MONTHLY;INTERVAL=1;BYDAY=2TU,4TU
    ```

    -   Это означает: каждый месяц во второй и четвертый вторник.


### <span class="section-num">5.2</span> Использование мастера {#использование-мастера}
