...
lastmod: 2026-09-11T20:41:00+03:00
tags: ["pdf"]
...
## <span class="section-num">1</span> Примеры команд для обработки pdf {#index.ru}
...
### <span class="section-num">1.1</span> Создание PDF-файла из изображений {#создание-pdf-файла-из-изображений}
...
### <span class="section-num">1.2</span> Объединение PDF-файлов {#объединение-pdf-файлов}
...
### <span class="section-num">1.3</span> Извлечь текст из PDF {#извлечь-текст-из-pdf}
...
### <span class="section-num">1.4</span> Зашифровать PDF-файл {#зашифровать-pdf-файл}
...
### <span class="section-num">1.5</span> Извлечение изображений из PDF-файла {#извлечение-изображений-из-pdf-файла}
...
### <span class="section-num">1.6</span> Извлечение диапазона страниц из PDF, разделение многостраничного PDF-документа {#извлечение-диапазона-страниц-из-pdf-разделение-многостраничного-pdf-документа}


#### <span class="section-num">1.6.1</span> ghostscript {#ghostscript}
...
### <span class="section-num">1.7</span> Замена страниц в pdf {#замена-страниц-в-pdf}


#### <span class="section-num">1.7.1</span> qpdf {#qpdf}
...
#### <span class="section-num">1.7.2</span> pdftk {#pdftk}
...
### <span class="section-num">1.8</span> Наложить PDF (nup) {#наложить-pdf--nup}
...
### <span class="section-num">1.9</span> Разделить страницы {#разделить-страницы}


#### <span class="section-num">1.9.1</span> mutool {#mutool}
...
### <span class="section-num">1.10</span> Метаданные {#метаданные}


#### <span class="section-num">1.10.1</span> Проверка метаданных {#проверка-метаданных}
...
#### <span class="section-num">1.10.2</span> Записать метаданные {#записать-метаданные}
...
#### <span class="section-num">1.10.3</span> Удалить метаданные {#удалить-метаданные}
...
### <span class="section-num">1.11</span> Уменьшить размер pdf-файла (сжатие) {#уменьшить-размер-pdf-файла--сжатие}


#### <span class="section-num">1.11.1</span> Ghostscript {#ghostscript}
...
-   Можно сжать изображения:
    -   Понижение разрешения (Downsampling):
        -   `-dColorImageResolution=150` (для цветных);
        -   `-dGrayImageResolution=150` (для оттенков серого);
        -   `-dMonoImageResolution=150` (для черно-белых).
    -   Смена типа сжатия (например, сжатие с потерями (jpeg)):
        -   `-dColorImageDownsampleType=/Bicubic`;
        -   `-dColorImageFilter=/DCTEncode` (jpeg-сжатие).


### <span class="section-num">1.12</span> Уменьшить размер pdf-файла (оптимизация) {#уменьшить-размер-pdf-файла--оптимизация}


#### <span class="section-num">1.12.1</span> qpdf {#qpdf}
...
#### <span class="section-num">1.12.2</span> exiftool {#exiftool}
...
### <span class="section-num">1.13</span> Растеризовать PDF-файл {#растеризовать-pdf-файл}
...
### <span class="section-num">1.14</span> Добавить цифровую подпись в PDF {#добавить-цифровую-подпись-в-pdf}
...
### <span class="section-num">1.15</span> Аннотации {#аннотации}


#### <span class="section-num">1.15.1</span> Удаление аннотаций из PDF-файла {#удаление-аннотаций-из-pdf-файла}
...
### <span class="section-num">1.16</span> Добавьте номера страниц {#добавьте-номера-страниц}
...
### <span class="section-num">1.17</span> Добавить метки страниц {#добавить-метки-страниц}
...
### <span class="section-num">1.18</span> Закладки {#закладки}


#### <span class="section-num">1.18.1</span> Извлечь закладки {#извлечь-закладки}
...
#### <span class="section-num">1.18.2</span> Добавить закладки {#добавить-закладки}
...
### <span class="section-num">1.19</span> Шрифты {#шрифты}


#### <span class="section-num">1.19.1</span> Список шрифтов {#список-шрифтов}
...
1.  poppler
    ...
    -   Используем команду `pdffonts` чтобы узнать, какие шрифты используются в PDF-файле и встроены ли они в него или нет:
        ...
        pdffonts file.pdf
        ```
    -   В колонке `emb` будет стоять `yes`  для встроенных шрифтов.


#### <span class="section-num">1.19.2</span> Внедрить шрифты в файл {#внедрить-шрифты-в-файл}
...
1.  ghostscript
    ...
    -   Перегенерим файл с помощью ghostscript:
        ```shell
        gs -dNOPAUSE -dBATCH -sDEVICE=pdfwrite -dPDFSETTINGS=/prepress -dEmbedAllFonts=true -dSubsetFonts=true -sOutputFile=output.pdf input.pdf
        ```
    -   Можно заменить `/prepress` или `/default`  на один из следующих вариантов:
        -   `/screen` : низкое качество (72 DPI);
        -   `/ebook` : среднее качество (150 DPI);
        -   `/printer` : качество выше (300 DPI);
        -   `/prepress` : высокое качество.


### <span class="section-num">1.20</span> Восстановить повреждённый PDF-файл {#восстановить-повреждённый-pdf-файл}


#### <span class="section-num">1.20.1</span> ghostscript {#ghostscript}
...
#### <span class="section-num">1.20.2</span> poppler {#poppler}
...
#### <span class="section-num">1.20.3</span> mupdf {#mupdf}
...
### <span class="section-num">1.21</span> Стандарт PDF/A {#стандарт-pdf-a}

-   [Стандарт PDF/A]({{< relref "2021-07-30-pdf-a-standard" >}})


#### <span class="section-num">1.21.1</span> Преобразование PDF в стандарт PDF/A {#преобразование-pdf-в-стандарт-pdf-a}
...
#### <span class="section-num">1.21.2</span> Проверка соответствия PDF/A {#проверка-соответствия-pdf-a}
...
### <span class="section-num">1.22</span> Оглавление в PDF {#оглавление-в-pdf}

-   [Pdf. Оглавление. pdf.tocgen]({{< relref "2024-06-21-pdf-toc-pdf-tocgen" >}})
-   [Pdf. Оглавление. pdf-toc]({{< relref "2024-06-21-pdf-toc-pdf-toc" >}})


### <span class="section-num">1.23</span> Преобразование djvu в pdf {#преобразование-djvu-в-pdf}

-   [Преобразование djvu в pdf]({{< relref "2024-07-26-convert-djvu-pdf" >}})


### <span class="section-num">1.24</span> Разбить pdf по главам {#разбить-pdf-по-главам}
